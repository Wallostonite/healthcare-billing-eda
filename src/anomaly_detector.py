...
class AnomalyDetector:
    ...
    def summary(self) -> pd.DataFrame:
        ...
        for col, stats in self.results.items():
            # Skip non-column entries like "low_recovery" (from flag_low_recovery)
            if "iqr_flagged" not in stats:
                continue
            rows.append({
                "column":               col,
    ...
    def flag_low_recovery(self) -> "AnomalyDetector":
        """
        Flag patients whose bill_collection_rate sits in the statistical bottom tail.

        Uses the IQR lower fence (Q1 - 1.5×IQR) on bill_collection_rate.
        Any patient below this fence has an unusually low collection rate —
        they are at highest risk of write-off and should be prioritised for
        billing follow-up.

        Results stored in self.results["low_recovery"].

        Returns self.
        """
        col = "bill_collection_rate"
        if col not in self.df.columns:
            logger.warning("[ANOMALY] 'bill_collection_rate' column not found — skipping flag_low_recovery")
            return self
        ...
        low_mask = self.df[col] < lower_fence
        low_df   = self.df[low_mask].copy()

        self.results["low_recovery"] = {
            "lower_fence":    round(float(lower_fence), 3),
            "flagged_count":  int(low_mask.sum()),
            "flagged_pct":    round(float(low_mask.mean() * 100), 2),
            "mean_rate":      round(float(low_df[col].mean()), 3) if len(low_df) else 0.0,
            "max_rate":       round(float(low_df[col].max()), 3) if len(low_df) else 0.0,
        }

        logger.info(
            f"[ANOMALY] Low recovery: {low_mask.sum():,} patients "
            f"below fence {lower_fence:.2f}% "
            f"({low_mask.mean()*100:.1f}% of all patients)"
        )
        return self
